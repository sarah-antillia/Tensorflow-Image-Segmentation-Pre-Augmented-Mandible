...
class ImageMaskDatasetGenerator:
  ...
  def generate_image_files(self) : 
    ...
    jpg_files = glob.glob(self.images_dir + "/*.jpg")
  ...
  def generate_mask_files(self):
    jpg_files = glob.glob(self.masks_dir + "/*.jpg")
    jpg_files = sorted(jpg_files)
    index = 10000
    for jpg_file in jpg_files:
      index += 1
...
if __name__ == "__main__":
  try:
    output_dir = "./Augmented-Mandible-master"
    if os.path.exists(output_dir):
      shutil.rmtree(output_dir)
    os.makedirs(output_dir)

    images_dir = "./Mandible-master/images/"
    masks_dir  = "./Mandible-master/masks/"
